	...
	if _instance is None:
		if platform == "linux" or platform == "linux2":
			...
			if manager == 'gstreamer':
				try:
					from octoprint_WBCustom.astroprint.camera.v4l2.gstreamer import GStreamerManager
			...
			if _instance is None:
				from octoprint_WBCustom.astroprint.camera.v4l2.mjpeg import MjpegManager
				_instance = MjpegManager()

		elif platform == "darwin":
			from octoprint_WBCustom.astroprint.camera.mac import CameraMacManager
...
class CameraManager(object):
	...
	def addPhotoToTimelapse(self, timelapseId, pasync=True):
		#Build text
	...
	def addPhotoToActiveTimelapse(self, pasync=True):
		if self.timelapseInfo:
			self.addPhotoToTimelapse(self.timelapseInfo['id'], pasync)
